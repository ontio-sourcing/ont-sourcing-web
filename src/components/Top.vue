...
export default {
    ...
    created(){
        ...
        "pageNum": 1,"pageSize": 10})
            .then(function (response) {
                ...
                that.listContent = response.data.result;
                that.listContent.forEach(item => {
    ...
    methods:{
        ...
        search(){
            var that = this;
            // that.showLoading = !this.showLoading;
            if(this.inputvalue != ''){
                ...
                .then(function (response) {
                    that.loading = false;
                    that.searchContent = response.data.result;
                    that.searchContent.forEach(item =>{
                        item._txhash=item.txhash.substring(0,10)+'.....'+item.txhash.substring(item.txhash.length-5);
                        item._ontId = item.ontid.substring(0,10)+'.....'+item.ontid.substring(item.ontid.length-5);
                    })
